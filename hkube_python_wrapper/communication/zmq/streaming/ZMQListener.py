...
class ZMQListener(object):
    ...
    def __init__(self, remoteAddress, onMessage, onReady, onNotReady, encoding, consumerType):
    ...
    def worker_socket(self, remoteAddress, context=None):
        ...
        self.send(worker, PPP_INIT)
        return worker

    def send(self, worker, signal, result=PPP_EMPTY):
        if(worker):
            arr = [signal, self.consumerType, result]
            signalStr = signals.get(signal)
            if(signalStr and shouldPrint):
                print('---- sending {signal} from {address} ----'.format(signal=signalStr, address=self.remoteAddress))
    ...
    def notReady(self):
        ...
        if (self._ready is False and not self._notReadySent):
            ...
            self.send(self.worker, PPP_NOT_READY)
