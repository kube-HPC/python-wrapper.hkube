...
class Tracer(object):
    ...
    def __init__(self, tracer_config):
        ...
        config=tracer_config.get('config', defaultConfig) if tracer_config is not None else defaultConfig
        service_name=tracer_config.get('service_name', 'algorunner') if tracer_config is not None else 'algorunner'
        tracerConfig = Config(
            config=config,
